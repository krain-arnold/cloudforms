class ServiceCatalogsImportExport
  ...
  def import(filename)
    ...
    ServiceTemplateCatalog.transaction do
      import_service_template_catalogs(contents[:catalogs])
    end
    ServiceTemplate.transaction do
      import_service_templates(contents[:templates])
    end
  end
  ...
  def export(filename)
    ...
    File.write(filename, {:catalogs => catalogs_hash, :templates => templates_hash}.to_yaml)
  end
...
namespace :redhat do
  namespace :service_catalogs do
    ...
    task :usage => [:environment] do
      puts 'Export - Usage: rake redhat:service_catalogs:export[/path/to/export]'
      puts 'Import - Usage: rake redhat:service_catalogs:import[/path/to/export]'
    end

    desc 'Import all dialogs from a YAML file'
    task :import, [:filename] => [:environment] do |_, arguments|
      ServiceCatalogsImportExport.new.import(arguments[:filename])
    end

    desc 'Exports all dialogs to a YAML file'
    task :export, [:filename] => [:environment] do |_, arguments|
      ServiceCatalogsImportExport.new.export(arguments[:filename])
    end
